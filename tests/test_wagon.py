...
class TestCreate(testtools.TestCase):
    ...
    def test_create_with_requirements(self):
        ...
        _invoke_click('create_wagon', [test_package], params)

        self.platform = 'any'
        name, version = wagon._get_name_and_version_from_setup(test_package)
        self.archive_name = wagon._set_archive_name(
            name,
            version,
            self.python_versions,
...
class TestValidate(testtools.TestCase):
    ...
    def test_fail_validation_exclude_and_missing_wheel(self):
        ...
        _invoke_click('create_wagon', [test_package], params)
        name, version = wagon._get_name_and_version_from_setup(test_package)
        archive_name = wagon._set_archive_name(
            name,
            version,
            [wagon._get_python_version()],
            'any')
